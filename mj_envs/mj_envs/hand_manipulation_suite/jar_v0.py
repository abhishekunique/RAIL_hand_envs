...
class JarEnvV0(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        ...
        mujoco_env.MujocoEnv.__init__(self, curr_dir+'/assets/DAPG_jar.xml', 5)
    ...
    def step(self, a):
        ...
        obj_angle = self.sim.data.qpos[-1]
        target_angle = -np.pi/2
        # pos cost
        dist = np.linalg.norm(obj_angle - target_angle)
        reward = -dist
        done = False
    ...
    def reset_model(self):
        ...
        # desired_orien[0] = -np.pi/2 #self.np_random.uniform(low=-1, high=1)
        # desired_orien[2] = np.pi/2
